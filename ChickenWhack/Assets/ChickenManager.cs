...
public class ChickenManager : MonoBehaviour
{
    ...
    public void SpawnChickens(int amount)
    {
        for(int i = 0; i < amount; i++)
        {
            ...
            chicken.onTakedown += SpawnExplosion;
            bool spawned = false;
            do
            {
                spawned = chicken.Spawn(Quaternion.AngleAxis(360f * Random.value, Vector3.up) * Vector3.forward * Random.Range(chickenSpawnMinRadius, chickenSpawnMaxRadius));
            }
            while (!spawned);
        }
    }
